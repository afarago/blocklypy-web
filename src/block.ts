...
export class Block {
  ...
  get_input(name: string) {
    //!! maybe add a variables_convert_fn callback
    try {
      ...
      if (input_type === 2) {
        ...
        // string = shadow block, [12, varname:"var1", monitor_id:"Xnr;(Ed)FB3`X+OV2PZ{"], [13, varname:"list1", monitor_id:"mXkX.MRn#+ItD^(Y$xs"]
        const ref = input[1];
        const ref_type = typeof ref;
        if (ref_type === 'string') {
          const block2 = this.getById(ref);
          //!!! const op = Block.ProcessOperation(block2);
          return op;
        ...
        if (is_reference) {
          ...
          const first_field = Object.values(block2?._block.fields)[0] as any[];
